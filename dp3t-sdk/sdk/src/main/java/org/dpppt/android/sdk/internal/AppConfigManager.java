...
import org.dpppt.android.sdk.util.FileUtil;
...
public class AppConfigManager {
    ...
    public void triggerLoad() {
        useDiscovery = true;
        ApplicationsList applicationsList = FileUtil.getFile(contextWeakReference.get(), isDevDiscoveryMode ? "discovery_dev.json" : "discovery.json");
        String json = Json.toJson(applicationsList);
        sharedPrefs.edit().putString(PREF_APPLICATION_LIST, json).apply();
    }
    ...
    public void updateFromDiscoverySynchronous() throws IOException {
        if (useDiscovery) {
            ApplicationsList response = FileUtil.getFile(contextWeakReference.get(), isDevDiscoveryMode ? "discovery_dev.json" : "discovery.json");
